...
subString start end str 
    | start < 0 = take end str
    | otherwise = take (end - start) (drop start str)
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (head (go 0 0 list)) ++ " elements and lower than " ++ show (last (go 0 0 list)) ++ " elements"
    where
        go :: Int -> Int -> [Int] -> [Int]
        go greater smaller l
         | null l = [greater, smaller]
         | head l < n = go (greater + 1) smaller (tail l)
         | head l > n = go greater (smaller+1) (tail l)
